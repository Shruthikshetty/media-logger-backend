 ...
 */
const deleteTvShow = (tvShowId, session) =>
  __awaiter(void 0, void 0, void 0, function* () {
    ...
    if (seasons.length > 0) {
      ...
      for (const season of seasons) {
        const deletedEpisodes = yield tv_episode_1.default.deleteMany(
          { season: season._id },
          { session }
        );
      ...
      const deletedSeason = yield tv_season_1.default.deleteMany(
        { tvShow: tvShowId },
        { session }
      );
    ...
    const deletedTvShow = yield tv_show_mode_1.default
      .findByIdAndDelete(tvShowId, { session })
      .lean()
      .exec();
    ...
    if (!deletedTvShow) {
      throw new api_error_1.ApiError(
        404,
        `TV show with id ${tvShowId} not found`
      );
    }
    return {
      deletedCount: {
