 ...
 */
const addSingleTvShow = (tvShowData, session) =>
  __awaiter(void 0, void 0, void 0, function* () {
    //destructure the season and rest of the tv show details
    const { seasons } = tvShowData,
      restTvDetails = __rest(tvShowData, ['seasons']);
    ...
    if (seasons && seasons.length > 0) {
      for (const seasonData of seasons) {
        ...
        const { episodes } = seasonData,
          seasonDetails = __rest(seasonData, ['episodes']);
        // create a new season
        const newSeason = new tv_season_1.default(
          Object.assign({ tvShow: saveTvShow._id }, seasonDetails)
        );
        // save the season
        ...
        if (!savedSeason) {
          throw new api_error_1.ApiError(
            500,
            `${seasonData.title} creation failed`
          );
        }
        ...
        if (episodes && episodes.length > 0) {
          for (const episodeData of episodes) {
            ...
            const newEpisode = new tv_episode_1.default(
              Object.assign({ season: savedSeason._id }, episodeData)
            );
            // save the episode
            ...
            if (!savedEpisode) {
              throw new api_error_1.ApiError(
                500,
                `${episodeData.title} creation failed`
              );
            }
     ...
     * }
     */
    return Object.assign(Object.assign({}, saveTvShow.toObject()), {
      seasons: savedSeasons.map((season) =>
        Object.assign(Object.assign({}, season.toObject()), {
